...
(function(){
  ...
  const blobCache = new Map();
   ...
   // === ПОДЕЛИТЬСЯ — РАБОТАЕТ С ЛЮБЫМ ПЛЕЕРОМ ===
document.addEventListener('DOMContentLoaded', () => {
  const shareBtn = document.querySelector('.header-actions .btn--ghost:last-child'); // Поделиться в шапке
  ...
  shareBtn.onclick = () => {
    // Находим активный плеер (с фокусом или первый с видео)
    const activePlayer = document.querySelector('.player.active') 
                       || document.querySelector('.player .url-input:not(:placeholder-shown)')?.closest('.player')
                       || document.querySelector('.player');

    if (!activePlayer) {
      alert('Нет активного плеера!');
      return;
    }

    const urlInput = activePlayer.querySelector('.url-input');
    const url = urlInput?.value.trim();

    if (!url) {
      alert('Вставь ссылку на YouTube в активный плеер!');
      return;
    }

    // Проверяем, что это YouTube
    if (!url.includes('youtube.com') && !url.includes('youtu.be')) {
      alert('Только YouTube-ссылки!');
      return;
    }

    const shareUrl = `${location.origin}${location.pathname}?share=${encodeURIComponent(url)}`;

    if (navigator.share) {
      navigator.share({ url: shareUrl }).catch(() => {});
    } else {
      navigator.clipboard.writeText(shareUrl).then(() => {
        alert('Ссылка скопирована!\nДруг увидит твой плейлист — НАВСЕГДА.');
      }).catch(() => {
        prompt('Скопируй вручную:', shareUrl);
      });
    }
  };
});

// === АВТОЗАГРУЗКА ИЗ ?share= (должна быть в app.js) ===
window.addEventListener('load', () => {
  ...
  if (sharedUrl) {
    ...
    // Находим первый плеер
    const firstPlayer = document.querySelector('.player');
    if (firstPlayer) {
      const urlInput = firstPlayer.querySelector('.url-input');
      if (urlInput) {
        urlInput.value = decoded;
        urlInput.dispatchEvent(new Event('input')); // если у тебя есть обработчик
