// Hoso Studio — YouTube НА ТВОЁМ САЙТЕ У ДРУГА — НАВСЕГДА
// ?share=ID — ССЫЛКА РАБОТАЕТ ВСЕГДА, НЕ СБРАСЫВАЕТСЯ НИКОГДА
...
(function(){
  ...
  // === КНОПКА ПОДЕЛИТЬСЯ — ССЫЛКА НАВСЕГДА ===
  shareBtn.onclick = () => {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(url).then(() => {
        alert('Ссылка скопирована!\nДруг увидит YouTube на твоём сайте — НАВСЕГДА, БЕЗ СБРОСА.');
  ...
  class Player {
    ...
    // === YouTube НА ТВОЁМ САЙТЕ — В ПЛЕЕРЕ ===
    async play(i) {
  ...
  // === ВОССТАНОВЛЕНИЕ — НЕ СБРАСЫВАЕТСЯ НИКОГДА ===
  const restore = async () => {
    await DB.open();

    // === КОПИРУЕМ ОТ АВТОРА — НАВСЕГДА У ДРУГА ===
    if (isShared && !localStorage.getItem(storageKey)) {
      const originalRaw = localStorage.getItem(originalKey);
      if (originalRaw) {
        localStorage.setItem(storageKey, originalRaw); // ПОЛНАЯ КОПИЯ НАВСЕГДА
      }
    }
