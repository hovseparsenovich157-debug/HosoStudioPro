...
(function(){
  ...
  // Проверка: если кнопка не найдена
  if (!shareBtn) {
    console.error('Кнопка #shareBtn не найдена в HTML!');
    return;
  }
  if (!youtubeInput) {
    console.error('Поле #youtubeLink не найдено!');
    return;
  }

  // === ТОЛЬКО ТЕПЕРЬ безопасно вешать обработчик ===
  shareBtn.onclick = () => {
    const youtubeUrl = youtubeInput.value.trim();
    
    if (!youtubeUrl) {
      alert('Сначала вставь ссылку на YouTube!');
      return;
    }

    const shareParam = encodeURIComponent(youtubeUrl);
    const shareUrl = `${location.origin}${location.pathname}?share=${shareParam}`;
