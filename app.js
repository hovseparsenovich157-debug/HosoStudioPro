// Hoso Studio — ССЫЛКА ?share=ID — КОПИРУЕТ ПЛЕЙЛИСТ НАВСЕГДА
// ОТКРЫВАЕТСЯ ВСЕГДА, НЕ ОДИН РАЗ
...
(function(){
  ...
  const shareId = urlParams.get('share'); // ID от автора

  // === ЕСЛИ ЕСТЬ ?share=ID — ЭТО КОПИЯ ОТ ДРУГОГО ===
  const isShared = !!shareId && shareId !== userId;

  // === ИТОГОВЫЙ КЛЮЧ ДЛЯ ХРАНЕНИЯ ===
  ...
  shareBtn.onclick = () => {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(url).then(() => {
        alert('Ссылка скопирована!\nДруг откроет — увидит твой плейлист НАВСЕГДА.');
  ...
  // === ВОССТАНОВЛЕНИЕ + КОПИЯ ОТ ССЫЛКИ ===
  const restore = async () => {
    await DB.open();
    let raw = localStorage.getItem(storageKey);

    // === ЕСЛИ ЭТО КОПИЯ (shareId) И ЕЩЁ НЕТ ДАННЫХ ===
    if (isShared && !raw) {
      const originalKey = `hoso_data_${shareId}`;
      const originalRaw = localStorage.getItem(originalKey);
      if (originalRaw) {
        raw = originalRaw;
        localStorage.setItem(storageKey, raw); // КОПИРУЕМ НАВСЕГДА
      }
    }
